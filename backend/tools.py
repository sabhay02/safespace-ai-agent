#set up olama with medgamma tools
import ollama
...
#setup twilio using api 
from twilio.rest import Client
from backend.config import TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER, EMERGENCY_CONTACT
def call_emergency(user_number:str) -> str:
    """
    Calls the emergency contact number using Twilio API.
    """
    try:
        ...
        return f"Emergency contact has been notified. Call SID: {call.sid}"
    except Exception as e:
        return f"Failed to call emergency contact: {str(e)}"
